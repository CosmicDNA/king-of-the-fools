...
const error2Pattern = /Error in plugin .*:.*\n.*\n.*/g
const acceptedError2 = { plugin: '@nomiclabs/hardhat-etherscan', info: 'Already Verified' }
...
const areErrorsAcceptable = (errors) => {
  return errors.map(error => {
    ...
    if (error2Pattern.test(error)) {
      const plugin = error.match(pluginNamePattern)
      const info = error.match(infoPattern)
      if (plugin[0] === acceptedError2.plugin && info[1] === acceptedError2.info) {
        return true
      }
    }
    return false
